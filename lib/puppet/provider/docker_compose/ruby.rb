Puppet::Type.type(:docker_compose).provide(:ruby) do
  ...
  def exists?
    ...
    counts = Hash[*compose_file.each_pair.collect { |key, value|
  ...
  def create
    ...
    # Added the support for service_name so that we can spawn specific container service.
    # service_name is the addition option
    args = ['-f', name, 'up', '-d', resource[:service_name]].insert(2, resource[:options]).compact
    dockercompose(args)
    if resource[:scale]
